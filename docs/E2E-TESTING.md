...
| افزودن اپراتور | `/api/admin/operators` POST | ✅ موفق | شماره موبایل و دسته‌بندی مجاز |
| ویرایش اپراتور | تغییر درصد پورسانت/دسته‌ها | ✅ موفق | `commission_rate` و `assigned_modules` اعمال می‌شوند |
| حذف اپراتور | `/api/admin/operators` DELETE | ✅ موفق | نقش به `CUSTOMER` برمی‌گردد |
...
| ویرایش قیمت خدمات | `/api/cms` upsert services | ✅ موفق | قیمت و تخفیف در دیتابیس ذخیره می‌شوند |
...
| ثبت درخواست برداشت | `/api/wallet/withdraw` | ✅ موفق | مبلغ از کیف پول کسر می‌شود |
| تأیید/رد درخواست برداشت | `/api/admin/withdrawals/[id]/approve\|reject` | ✅ موفق | رسید و یادداشت مدیر ثبت می‌شود |
| مدیریت منابع RSS | `/api/admin/news-feeds` | ✅ موفق | افزودن/حذف منابع خبری |
| مدیریت تنظیمات بازی | `/api/admin/game-config` | ✅ موفق | افزودن/حذف کلید-مقدار |
